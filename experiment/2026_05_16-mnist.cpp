...
int main(int, char const *const *const) {
	...
	Network::FeedForward<CF> network(
		{make_shared<Activation::Linear<CF>>(
			 Tensor<CF, 2>({784, 1024}, gen, dist),
			 Tensor<CF, 1>({1024}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Normalization<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({1024, 384}, gen, dist),
				Tensor<CF, 1>({384}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Normalization<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({384, 128}, gen, dist),
				Tensor<CF, 1>({128}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Normalization<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({128, 48}, gen, dist),
				Tensor<CF, 1>({48}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Normalization<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({48, 10}, gen, dist),
		...
		RF(k, 0, C_EPOCH) {
...

			{
				...
				ofstream encoderStream(
					assetPath / ".data" /
					(string("network.0.epoch.") + to_string(k) +
						".hfm"));
