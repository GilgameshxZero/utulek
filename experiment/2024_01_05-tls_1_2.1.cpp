...
template<
	std::size_t sizeRepBytes,
	template<typename> class... BlockMembers>
class TlsBlockSized :
	public TlsBlockBasic<BlockMembers...> {
	public:
	using TlsBlockBasic<BlockMembers...>::TlsBlockBasic;
	operator std::string() const {
		std::string basicStr{
			static_cast<TlsBlockBasic<BlockMembers...>>(*this)};
		return basicStr;
	}
};
class TlsBlockArray;
...
// Serialize byte data at `block` into `buffer`. Guaranteed
// to write `sizeof(Block)` bytes. Returns number of bytes
// written.
...
template<
	template<typename> class BlockMember,
	template<typename> class... BlockMembers>
std::size_t serializeTlsBlock(
	TlsBlockBasic<BlockMember, BlockMembers...> const &block,
	char *buffer) {
	buffer += serializeTlsBlock(
		block.template get<BlockMember>(), buffer);
	serializeTlsBlock(
...

	{
		...
		tlsPlaintext
			...
			<< "\x00\x0B\x00\x02\x01\x00"s // Extension - EC point
																		 // formats.
			<< "\x00\x0A\x00\x04\x00\x02\x00\x17"s // Extension -
																						 // Elliptic
																						 // curves.
			;
