...
int main(int, char const *const *const) {
	...
	Network::FeedForward<LD> network(
		{make_shared<Activation::Linear<LD>>(
			 Tensor<LD, 2>({784, 1024}, gen, dist),
			 Tensor<LD, 1>({1024}, gen, dist)),
			make_shared<Activation::Relu<LD>>(),
			make_shared<Activation::Normalization<LD>>(),
			make_shared<Activation::Linear<LD>>(
				Tensor<LD, 2>({1024, 384}, gen, dist),
				Tensor<LD, 1>({384}, gen, dist)),
			make_shared<Activation::Relu<LD>>(),
			make_shared<Activation::Normalization<LD>>(),
			make_shared<Activation::Linear<LD>>(
				Tensor<LD, 2>({384, 128}, gen, dist),
				Tensor<LD, 1>({128}, gen, dist)),
			make_shared<Activation::Relu<LD>>(),
			make_shared<Activation::Normalization<LD>>(),
			make_shared<Activation::Linear<LD>>(
				Tensor<LD, 2>({128, 36}, gen, dist),
				Tensor<LD, 1>({36}, gen, dist)),
			make_shared<Activation::Relu<LD>>(),
			make_shared<Activation::Normalization<LD>>(),
			make_shared<Activation::Linear<LD>>(
				Tensor<LD, 2>({36, 10}, gen, dist),
