...
//-----------------------------------------------------------------------------
	...
	#define OLS_DRIVER_FILE_NAME_WIN_NT \
		_T(".WinRing0.tmp.sys")
	#define OLS_DRIVER_FILE_NAME_WIN_NT_X64 \
		_T(".WinRing0x64.tmp.sys")
...
// Generated with `xxd -i WinRing0x64.sys
// WinRing0x64.sys.hpp`.
#include "2025_07_26-ec_pin.cpp.assets/WinRing0x64.sys.hpp"

std::string const MUTEX_NAME_BASE{
	"utulek/experiment/2025_07_26-ec_pin.cpp"},
...
int main(int argc, const char *const *argv) {
	...
	// Hide console window.
	ShowWindow(
		validateSystemCall(GetConsoleWindow()), SW_HIDE);

	// Wait for previous instance to exit, if exists.
	HANDLE mutexHold{validateSystemCall(
		CreateMutex(NULL, FALSE, MUTEX_NAME_HOLD.c_str()))},
		mutexQuit{validateSystemCall(
			CreateMutex(NULL, FALSE, MUTEX_NAME_QUIT.c_str()))};
	...
	unsigned long registerId{0x2d}, registerValue{0x88},
		intervalMs{4000};
