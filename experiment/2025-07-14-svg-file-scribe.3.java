...
public class SvgFileScribe {
	...
	private Context scribbleActivity;
	private Uri uri;
	...
	public SvgFileScribe(Callback callback, Context scribbleActivity, Uri uri,
						...
						while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
							...
							if (parser.getName().equals("g")) {
								...
								if (transform != null) {
									...
									cx = Float.parseFloat(data[4]);
									cy
										= Float
											.parseFloat(data[5].substring(0, data[5].length() - 1));
									XenaApplication.log("HEREHEREHERE ", cx, " ", cy);
								}
							}
							...
							lastPoint.x
								= (cx + Float.parseFloat(buffer.toString()))
									/ SvgFileScribe.COORDINATE_SCALE_FACTOR * additionalScale;
							buffer.setLength(0);
							...
							lastPoint.y
								= (cy - Float.parseFloat(buffer.toString()))
									/ SvgFileScribe.COORDINATE_SCALE_FACTOR * additionalScale;
							buffer.setLength(0);
							...
							for (; i < d.length();) {
								...
								if (mode == 'l') {
									...
									lastPoint.x
										= pointDelta.x + cx / SvgFileScribe.COORDINATE_SCALE_FACTOR
											* additionalScale;
									lastPoint.y
										= pointDelta.y + cy / SvgFileScribe.COORDINATE_SCALE_FACTOR
											* additionalScale;
								} else if (mode == 'H') {
									lastPoint.x = pointDelta.x;
								} else if (mode == 'V') {
