...
int main() {
	...
	// Too low and gradient doesn't reach through the entire
	// network. Too high and gradients explode and become
	// directionless (?)
	normal_distribution<LD> dist(0.0L, 8e-2);
	...
	Network::FeedForward<CF> network(
		{make_shared<Activation::Linear<CF>>(
			 Tensor<CF, 2>({3072, 1024}, gen, dist),
			 Tensor<CF, 1>({1024}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({1024, 256}, gen, dist),
				Tensor<CF, 1>({256}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({256, 96}, gen, dist),
				Tensor<CF, 1>({96}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({96, 32}, gen, dist),
				Tensor<CF, 1>({32}, gen, dist)),
			make_shared<Activation::Relu<CF>>(),
			make_shared<Activation::Linear<CF>>(
				Tensor<CF, 2>({32, 10}, gen, dist),
		...
		RF(k, 0, C_EPOCH) {
				...
				RF(i, 0, cBatchTrain) {
					...
					RF(j, 0, C_THREAD) {
						tp.queueTask([&]() {
							...
							// cout <<
							// activationGradientV[jInner].back()[9] <<
							// endl;
...

			{
				...
				ofstream encoderStream(
					assetPath / ".data" /
					(string("c10.2-network-epoch_") + to_string(k) +
						".hfm"));
