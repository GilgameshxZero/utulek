...
namespace oof {
	// Feel free to bit_cast, reinterpret_cast or memcpy your
	// 3-byte color type into this.
	struct color {
	...
	[[nodiscard]] auto fg_color(const color &col)
		-> fg_rgb_color_sequence;

	// Sets the foreground indexed color. Index must be in [1,
	// 255]. You can define colors with set_index_color().
	[[nodiscard]] auto fg_color(int index)
		-> fg_index_color_sequence;

	// Sets the background RGB color
	[[nodiscard]] auto bg_color(const color &col)
		-> bg_rgb_color_sequence;

	// Sets the background indexed color. Index must be in [1,
	// 255]. You can define colors with set_index_color().
	[[nodiscard]] auto bg_color(int index)
		-> bg_index_color_sequence;

	// Sets the indexed color. Index must be in [1, 255].
	[[nodiscard]] auto set_index_color(
		int index,
		const color &col) -> set_index_color_sequence;

	// Sets the underline state of the console
	[[nodiscard]] auto underline(bool new_value = true)
		-> underline_sequence;

	// Sets the bold state of the console. Warning: Bold is
	// not supported by all console, see readme
	[[nodiscard]] auto bold(bool new_value = true)
		-> bold_sequence;

	// Sets cursor visibility state. Recommended to turn off
	// before doing real-time displays
	[[nodiscard]] auto cursor_visibility(bool new_value)
		-> cursor_visibility_sequence;

	// Resets foreground- and background color, underline and
	// bold state
	[[nodiscard]] auto reset_formatting() -> reset_sequence;

	// Clears the screen
	[[nodiscard]] auto clear_screen()
		-> clear_screen_sequence;

	// Sets the cursor position. Zero-based
	[[nodiscard]] auto position(int line, int column)
		-> position_sequence;
	[[nodiscard]] auto vposition(int line)
		-> vposition_sequence;
	[[nodiscard]] auto hposition(int column)
		-> hposition_sequence;
	[[nodiscard]] auto store_position()
		-> store_position_sequence;
	[[nodiscard]] auto load_position()
		-> load_position_sequence;

	// Moves the cursor a certain amount
	[[nodiscard]] auto move_left(int amount)
		-> move_left_sequence;
	[[nodiscard]] auto move_right(int amount)
		-> move_right_sequence;
	[[nodiscard]] auto move_up(int amount)
		-> move_up_sequence;
	...
	// Returns the exact size a string from this vector of
	// sequence types
	[[nodiscard]] auto get_string_reserve_size(
		const std::vector<sequence_variant_type> &sequences)
		-> size_t;
	...
	struct screen {
		...
		// This constructor taking a fill_char implies black
		...
		// This constructor taking a fill_char implies black
		// background, white foreground color and top left start
		explicit screen(
			int width,
			int height,
			char_type fill_char);
		...
		[[nodiscard]] auto get_cell(int column, int line)
			-> cell<string_type> &;
		[[nodiscard]] auto is_inside(int column, int line) const
			-> bool;
		...
		mutable std::vector<sequence_variant_type>
			m_sequence_buffer;
	};
	...
	struct pixel_screen {
		...
		// This will init with black fill color and starting at
		// the top left
	...
	auto operator<<(
		stream_type &os,
		const sequence_type &sequence) -> stream_type &;
	...
	namespace detail {
		...
		auto write_int_to_string(
			...
			const bool with_leading_semicolon) -> void;
		...
		struct cell_pos {
			...
			[[nodiscard]] constexpr auto operator+(
				const int jump_amount) const -> cell_pos {
		...
		struct draw_state {
			...
			auto write_sequence(
				...
				const std::optional<
					std::reference_wrapper<const cell_type>>
					&old_cell_state,
			...
			private:
			[[nodiscard]] auto is_position_sequence_necessary(
...
auto oof::operator<<(
	...
	temp_string.reserve(
		detail::get_sequence_string_size(sequence));
...
auto oof::detail::write_int_to_string(
	...
	if (value >= 10)
		target +=
			static_cast<char_type>('0' + (value % 100) / 10);
	target += '0' + value % 10;
}
...
template <
	...
	typename T,
	typename... Ts>
auto oof::detail::write_ints_into_string(
	string_type &target,
	const T &first,
	const Ts &...rest) -> void {
...
auto oof::write_sequence_into_string(
	...
	if constexpr (std::is_same_v<
		...
		if constexpr (std::same_as<
			...
			std::
				same_as<sequence_type, store_position_sequence> ||
...
auto oof::screen<string_type>::update_sequence_buffer()
	...
	for (detail::cell_pos relative_pos{
		...
		std::optional<
			std::reference_wrapper<const cell<string_type>>>
			old_cell_state;
		if (this->m_old_cells.empty() == false)
			old_cell_state.emplace(
				this->m_old_cells[relative_pos.m_index]);
...
auto oof::screen<string_type>::get_string() const
	...
	string_type result =
		::oof::get_string_from_sequences<string_type>(
			m_sequence_buffer);
	m_old_cells = m_cells;
	return result;
}
...
auto oof::screen<string_type>::get_string(
	...
	// Reserve if the string buffer is still empty (on the
	// first call)
	if (buffer.empty())
		buffer.reserve(
			::oof::get_string_reserve_size(m_sequence_buffer));

	buffer.clear();

	::oof::detail::write_sequence_string_no_reserve(
		m_sequence_buffer, buffer);
	m_old_cells = m_cells;
}
...
auto oof::screen<string_type>::write_into(
	...
	const int ending_column =
		column + static_cast<int>(text.size());
	if (ending_column > m_width) {
		::oof::detail::error(
			"Trying to write_into() with a text that won't fit.");
		return;
	}
	for (size_t i = 0; i < text.size(); ++i) {
		cell<string_type> &cell =
			m_cells[line * m_width + column + i];
...
auto oof::screen<string_type>::is_inside(
	...
	return column >= 0 && column < m_width && line >= 0 &&
		line < m_height;
}
...
auto oof::get_string_reserve_size(
	const std::vector<sequence_variant_type> &sequences)
	-> size_t {
	size_t reserve_size{};
	for (const sequence_variant_type &sequence : sequences)
		std::visit(
			[&](const auto &alternative) {
				reserve_size +=
					detail::get_sequence_string_size(alternative);
			},
			sequence);
	return reserve_size;
}
...
auto oof::get_string_from_sequences(
	...
	result_str.reserve(
		::oof::get_string_reserve_size(sequences));
	::oof::detail::write_sequence_string_no_reserve(
		sequences, result_str);
	return result_str;
}
...
auto oof::position(const int line, const int column)
	-> position_sequence {
...
auto oof::vposition(const int line) -> vposition_sequence {
	return vposition_sequence{
		.m_line = static_cast<uint8_t>(line)};
}

auto oof::hposition(const int column)
	-> hposition_sequence {
	return hposition_sequence{
		.m_column = static_cast<uint8_t>(column)};
}
...
auto oof::move_left(const int amount)
	-> move_left_sequence {
	return move_left_sequence{
		.m_amount = static_cast<uint8_t>(amount)};
}

auto oof::move_right(const int amount)
	-> move_right_sequence {
	return move_right_sequence{
		.m_amount = static_cast<uint8_t>(amount)};
}

auto oof::move_up(const int amount) -> move_up_sequence {
	return move_up_sequence{
		.m_amount = static_cast<uint8_t>(amount)};
}

auto oof::move_down(const int amount)
	-> move_down_sequence {
	return move_down_sequence{
		.m_amount = static_cast<uint8_t>(amount)};
}

auto oof::fg_color(const color &col)
	-> fg_rgb_color_sequence {
	return fg_rgb_color_sequence{.m_color = col};
}

auto oof::fg_color(const int index)
	-> fg_index_color_sequence {
...
auto oof::set_index_color(const int index, const color &col)
	-> set_index_color_sequence {
	if (index < 1 || index > 255) {
		std::string msg = "Index must be in [1, 255], was: ";
		msg += std::to_string(index);
		::oof::detail::error(msg);
		return set_index_color_sequence{
			.m_index = 1, .m_color = col};
	}
	return set_index_color_sequence{
		.m_index = index, .m_color = col};
}

auto oof::bg_color(const color &col)
	-> bg_rgb_color_sequence {
	return bg_rgb_color_sequence{.m_color = col};
}

auto oof::bg_color(const int index)
	-> bg_index_color_sequence {
	if (index < 1 || index > 255) {
...
auto oof::underline(const bool new_value)
	-> underline_sequence {
...
auto oof::cursor_visibility(const bool new_value)
	-> cursor_visibility_sequence {
	return cursor_visibility_sequence{
		.m_visibility = new_value};
}
...
auto oof::pixel_screen::get_string(
	std::wstring &buffer) const -> void {
	compute_result();
...
auto oof::pixel_screen::get_line_height() const -> int {
	...
	const int last_line =
		(m_origin_halfline - 1 + m_halfline_height) / 2;
	return last_line - first_line + 1;
}

auto oof::pixel_screen::is_in(
	const int column,
	const int halfline) const -> bool {
	const size_t index =
		halfline * this->get_width() + column;
	return index >= 0 && index < m_pixels.size();
}

auto oof::pixel_screen::get_color(
	const int column,
	const int halfline) const -> const color & {
	const size_t index =
		halfline * this->get_width() + column;
...
auto oof::detail::draw_state<string_type>::write_sequence(
	...
	const int origin_line,
	const int origin_column) -> void {
	// As long as there's no difference from last draw, don't
	// do anything
	if (target_cell_state == old_cell_state) return;
	...
	if (m_format.has_value() == false) {
		...
		sequence_buffer.push_back(
			underline_sequence{
				.m_underline =
					target_cell_state.m_format.m_underline});
		sequence_buffer.push_back(
			bold_sequence{
				.m_bold = target_cell_state.m_format.m_bold});
	} else {
		// Apply differences between console state and the
		// target state
		if (
			target_cell_state.m_format.m_fg_color !=
			m_format->m_fg_color)
			sequence_buffer.push_back(
				fg_rgb_color_sequence{
					.m_color =
						target_cell_state.m_format.m_fg_color});
		if (
			target_cell_state.m_format.m_bg_color !=
			m_format->m_bg_color)
			sequence_buffer.push_back(
				bg_rgb_color_sequence{
					.m_color =
		...
		if (
			target_cell_state.m_format.m_underline !=
			m_format->m_underline)
			sequence_buffer.push_back(
				underline_sequence{
					.m_underline =
						target_cell_state.m_format.m_underline});
		if (
			target_cell_state.m_format.m_bold != m_format->m_bold)
			sequence_buffer.push_back(
				bold_sequence{
					.m_bold = target_cell_state.m_format.m_bold});
	}
	...
	if (this->is_position_sequence_necessary(target_pos)) {
		sequence_buffer.push_back(
			position_sequence{
				.m_line = static_cast<uint8_t>(
					target_pos.get_line() + origin_line),
				.m_column = static_cast<uint8_t>(
					target_pos.get_column() + origin_column)});
	}

	sequence_buffer.push_back(
		fitting_char_sequence_t<string_type>{
			.m_letter = target_cell_state.m_letter});
...
auto oof::detail::draw_state<string_type>::
	is_position_sequence_necessary(
		const cell_pos &target_pos) const -> bool {
	// There is was nothing written before, hence the cursor
	// position is unknown
	if (m_last_written_pos.has_value() == false) return true;

	const cell_pos current_cursor_pos =
		m_last_written_pos.value() + 1;

	// The cursor position is known to be wrong
	if (target_pos != current_cursor_pos) return true;

	// If we're on the "right" position according to the
	// subset of the buffer, the position still needs to be
	// set if there was a line jump.
	if (
		current_cursor_pos.get_line() !=
		m_last_written_pos->get_line())
		return true;
...
auto oof::detail::get_pixel_background(
	const color &fill_color) -> cell<std::wstring> {
...
// This is just to mass-instantiate the extender functions
// by abusing std::visit
auto impl_fun() -> void {
	std::visit(
		[](const auto &altern) {
