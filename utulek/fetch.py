...
def fetch_ds_cifar_10(cache_dir=".data/cifar-10"):
		...
		try:
			with TqdmByteTransfer() as tqdm_byte_transfer:
				urllib.request.urlretrieve(
					REMOTE_PATH,
					filename=LOCAL_PATH,
					reporthook=tqdm_byte_transfer.up_to,
				)
		except:
	...
	with tarfile.open(LOCAL_PATH) as tar:
		...
		with tar.extractfile(
			"cifar-10-batches-py/batches.meta") as file:
			obj = pickle.load(file, encoding="bytes")
			labels = np.array(obj[b"label_names"]).astype(str)

	X = np.vstack(Xs).reshape((-1, 3, 32, 32)).transpose(
		(0, 2, 3, 1))
